...
% Attempt on why is it new
%* there's debate about the value of transparent AI and if it is needed for Trust
%* some say it is (quote papers), some say it isn't (quote papers)
%* we want to find out if it is and we expect the answer to be it depends on the audience, especially how much AI experience you have, and if you are used to making decision that could have huge impact on other people like medical diagnosis


%New contribution: We focused specifically on medics no medics, and ai experts and no ai experts as well as high stakes decision making
...
Insights can be gained into the results of the survey by applying unsupervised methods. These do not rely on any labels or targets, as would be the case in supervised or semi-supervised methods, so avoid many of the biases that such methods would carry. In particular we apply Uniform Manifold Approximation and Projection (UMAP) \cite{McInnes2018} as dimensionality reduction, then Hierachical Density-Based Spatial Clustering And Noise (HDBSCAN) \cite{McInnes2018} as unsupervised clustering on the resulting UMAP embedding. 
...
We apply UMAP, then HDBSCAN to the resulting embedding, and present the results in Figures \ref{fig:embedding}, \ref{fig:cluster_means}.
...
\end{subfigure}
\caption{The results of unsupervised clustering with UMAP and HDBSCAN.}
\label{fig:test}
\end{figure}
